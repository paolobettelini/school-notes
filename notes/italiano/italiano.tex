...
\makecell[l]{
    ...
    \textit{non \colorbox{orange}{creder} quel ch'al suo dispetto \colorbox{orange}{crede}:} \\
    \textit{ch'altra Angelica sia, \colorbox{orange}{creder} si sforza,} \\
...
\makecell[l]{
    ...
    \textit{de la sua donna e \colorbox{orange}{crede} e \colorbox{orange}{brama} e \colorbox{orange}{spera},} \\
...
\centering
\makecell[l]{
    \textit{Afflitto e stanco al fin cade ne l'erba,} \\
    \textit{e ficca gli occhi al cielo, e non fa motto.} \\
    \textit{Senza cibo e dormir così si serba,} \\
    \textit{che 'l sole esce tre volte e torna sotto.} \\
    \textit{Di crescer non cessò la pena acerba,} \\
    \textit{che fuor del senno al fin l'ebbe condotto.} \\
    \textit{Il quarto dì, da gran furor commosso,} \\
    \textit{e maglie e piastre si stracciò di dosso.}
}
\end{minipage}
\end{center}
...
\centering
\makecell[l]{
    \textit{Qui riman l'elmo, e là riman lo scudo,} \\
    \textit{lontan gli arnesi, e più lontan l'usbergo:} \\
    \textit{l'arme sue tutte, in somma vi concludo,} \\
    \textit{avean pel bosco differente albergo.} \\
    \textit{E poi si squarciò i panni, e mostrò ignudo} \\
    \textit{l'ispido ventre e tutto 'l petto e 'l tergo;} \\
    \textit{e cominciò la gran follia, sì orrenda,} \\
    \textit{che de la più non sarà mai ch'intenda.}
}
\end{minipage}
\end{center}
...
\centering
\makecell[l]{
    \textit{In tanta rabbia, in tanto furor venne,} \\
    \textit{che rimase offuscato in ogni senso.} \\
    \textit{Di tor la spada in man non gli sovenne;} \\
    \textit{che fatte avria mirabil cose, penso.} \\
    \textit{Ma né quella, né scure, né bipenne} \\
    \textit{era bisogno al suo vigore immenso.} \\
    \textit{Quivi fe' ben de le sue prove eccelse,} \\
    \textit{ch'un alto pino al primo crollo svelse:}
}
\end{minipage}
\end{center}
...
\centering
\makecell[l]{
    \textit{e svelse dopo il primo altri parecchi,} \\
    \textit{come fosser finocchi, ebuli o aneti;} \\
    \textit{e fe' il simil di querce e d'olmi vecchi,} \\
    \textit{di faggi e d'orni e d'illici e d'abeti.} \\
    \textit{Quel ch'un ucellator che s'apparecchi} \\
    \textit{il campo mondo, fa, per por le reti,} \\
    \textit{dei giunchi e de le stoppie e de l'urtiche,} \\
    \textit{facea de cerri e d'altre piante antiche.}
}
\end{minipage}
\end{center}
...
\centering
\makecell[l]{
    \textit{I pastor che sentito hanno il fracasso,} \\
    \textit{lasciando il gregge sparso alla foresta,} \\
    \textit{chi di qua, chi di là, tutti a gran passo} \\
    \textit{vi vengono a veder che cosa è questa.} \\
    \textit{Ma son giunto a quel segno il qual s'io passo} \\
    \textit{vi potria la mia istoria esser molesta;} \\
    \textit{ed io la vo' più tosto diferire,} \\
    \textit{che v'abbia per lunghezza a fastidire.}
}
\end{minipage}
\end{center}
...
% Illuminismo, l'enciclopedia di Diderot
% il sapere è il mezzo per conseguire il gmilioramento della vita materiale e morale (progress
Una pena per essere deterrente, deve essere sempre più svantaggiosa dell'azione commessa.
% Pena pecuniaria e recidiva
